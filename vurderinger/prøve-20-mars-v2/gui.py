...
class App:
    ...
    def onOff(self) -> None:
        ...
        message = "på" if self.isOn else "av"
        if self.isOn:
            self.current.configure(text=f"Temperatur:\n{self.currentTemperatur}°C")
        else:
            self.current.configure(text=f"({message})")
        self.result.configure(text=f"Slått {message}")
    ...
    def create_gui(self) -> None:
        ...
        self.onOffButton.grid(row=0, column=0, rowspan=4, padx=5, pady=5, sticky="NS")
